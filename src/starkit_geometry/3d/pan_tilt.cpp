#include "starkit_geometry/3d/pan_tilt.h"
...
PanTilt::PanTilt(const Eigen::Vector3d& v)
{
  ...
  {
    if (v(2) == 0)
      throw std::runtime_error(DEBUG_INFO + ": norm of vector is 0");
    pan = Angle(0);
...
{
  return Eigen::Vector3d(cos(pan) * cos(tilt), sin(pan) * cos(tilt), -sin(tilt));
}
