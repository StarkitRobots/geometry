...
{
  Point diff = p - origin;
  return origin + direction * Point::dotProduct(diff, direction);
}
...
ParametricLine ParametricLine::average(const ParametricLine& l1, const ParametricLine& l2)
{
  return weightedAverage(l1, l2, 1.0, 1.0);
}
ParametricLine ParametricLine::weightedAverage(const ParametricLine& l1, const ParametricLine& l2, double weight1,
                                               double weight2)
{
  Point inter;
  try
...
Angle ParametricLine::angleDiff(const ParametricLine& l1, const ParametricLine& l2)
{
...
vector<ParametricLine> mergeLines(const vector<ParametricLine>& lines, double rhoTol, double thetaTol)
{
  ...
  for (unsigned int i = 0; i < lines.size(); i++)
  {
    ...
    for (unsigned int j = 0; j < mergedLines.size(); j++)
    {
      ...
      if (similarLines(newLine, oldLine, rhoTol, thetaTol))
      {
        ...
        ParametricLine result = ParametricLine::weightedAverage(newLine, oldLine, 1, score);
