...
starkit_utils::Angle Point::getTheta() const
{
  double theta = atan2(getY(), getX()) * 180 / M_PI;
  return starkit_utils::Angle(theta);
}

bool Point::isCollinear(const Point& other) const
{
  starkit_utils::Angle deltaTheta = getTheta() - other.getTheta();
...
Point Point::rotation(const starkit_utils::Angle& a) const
{
...
Point Point::rotation(const starkit_utils::Angle& a, const Point& rotCenter) const
{
...
Point Point::mkPointFromPolar(double rho, starkit_utils::Angle a)
{
...
ostream& operator<<(ostream& out, const starkit_geometry::Point& p)
{
...
starkit_geometry::Point average(const vector<starkit_geometry::Point>& points)
{
  ...
  return starkit_geometry::Point(xSum / n, ySum / n);
}

starkit_geometry::Point average(const vector<starkit_geometry::Point>& points, const vector<double>& weights)
{
  double xSum(0), ySum(0);
  ...
  return starkit_geometry::Point(xSum, ySum) / totalWeight;
}

double stdDev(const vector<starkit_geometry::Point>& points)
{
  starkit_geometry::Point middle = average(points);
  double squareDist(0);
  int n = points.size();
  for (int i = 0; i < n; i++)
  {
