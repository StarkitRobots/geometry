...
class CollinearException : public std::exception
{
  ...
  {
    return "Collinear objects unexpected\n";
  };
};
...
public:
  ...
  ParametricLine(const ParametricLine& other) : origin(other.origin), direction(other.direction){};
  ParametricLine(const Point& p1, const Point& p2) : origin(p1)
  {
    direction = p2 - p1;
  ...
  static ParametricLine average(const ParametricLine& l1, const ParametricLine& l2);
  static ParametricLine weightedAverage(const ParametricLine& l1, const ParametricLine& l2, double weight1,
                                        double weight2);

  // Return the smallest angle diff between two lines (-90 to 90)
  static starkit_utils::Angle angleDiff(const ParametricLine& l1, const ParametricLine& l2);

  static ParametricLine fromRhoTheta(double rho, const starkit_utils::Angle& theta);
};

// thetaTol: degrees
bool similarLines(const ParametricLine& l1, const ParametricLine& l2, double rhoTol, double thetaTol);
 ...
 */
std::vector<ParametricLine> mergeLines(const std::vector<ParametricLine>& lines, double rhoTol, double thetaTol);
std::vector<ParametricLine> mergeLines(const std::vector<ParametricLine>& lines, double rhoTol, double thetaTol,
                                       std::vector<unsigned int>& scores);

}  // namespace starkit_geometry

std::ostream& operator<<(std::ostream& out, const starkit_geometry::ParametricLine& p);
